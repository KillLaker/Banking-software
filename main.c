...
void main_menu(struct Account* account) {
    int option;
    if (account == NULL) {
        return;
    }
    while (1) {
        ...
        switch (option) {
            ...
            case 2:
                ...
                printf(ANSI_COLOR_BLUE);
                break;
            //case 3: system("cls"); transaction(account, account2)
            case 4:
                return;
            default:
                break;
...
void register_login_menu(struct Hashtable* table, const char* filename, const char* filenameRes) {
    int option;
...
int main()
{
    ...
    register_login_menu(table, filenameForTempFile, filenameForResultFile);
